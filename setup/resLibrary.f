...
MODULE resLibrary
    use Constants
! integer, parameter :: MAXRES = 50, MAXATPERRES = 30, NULL=0, COVB=1 !TODO Unificar constants
 type atom
...
10 read (unt, '(A80)', end=20) str
   ...
   if (args(1).eq.'RESIDUE') then
      ...
      if (trim(args(1)).eq.rlib%residues(i)%resId) then 
         ...
         rlib%residues(i)%atoms(j)%atomId = args(2)(1:4)
         rlib%residues(i)%atoms(j)%atType = args(3)(1:4)
            ...
            if (trim(args(k)).ne.'') then 
               do kk=1,rlib%residues(i)%natoms
                  if (eqId(rlib%residues(i)%atoms(kk)%atomId,trim(args(k)))) then
                     rlib%residues(i)%bonds(j,kk) = COVB
                     rlib%residues(i)%bonds(kk,j) = COVB     
                     ok=.true.
                  endif
               enddo
               if (.not.ok) then
