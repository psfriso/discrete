...
!===============================================================================
    ...
    subroutine markInterface(str, recNAts, dint0, dint1, irig, SetCoreCa, SetFrozen)
        ...
        do i = 1, str % natoms
            if (.not.str % ats(i) % dummy)then
                do j = 1, str % natoms
                    if (str % ats(j) % resNumPDB .eq. str % ats(i) % resNumPDB.and. &
                    str % ats(j) % molNum .eq. str % ats(i) % molNum) str % ats(j) % dummy = .false.
                enddo
            endif
            if (.not.str % ats(i) % frozen)then
                do j = 1, str % natoms
                    if (str % ats(j) % resNumPDB .eq. str % ats(i) % resNumPDB.and. &
                    str % ats(j) % molNum .eq. str % ats(i) % molNum) str % ats(j) % frozen = .false.
                enddo
            endif
            if (.not.str % ats(i) % dummy)then
                do j = str % natoms, 1, -1
    ...
    subroutine setIntraResBonds(str, resLib)
            ...
            do j = str % res(nr) % ini, str % res(nr) % fin - 1
                do k = j + 1, str % res(nr) % fin
                    if (.not.eqId(str % ats(k) % atomId, 'OXT')) then
                        ...
                        str % bonds(j, k) = rr % bonds(at1 % ind, at2 % ind) ! COVB heredat de resLibrary.f
                    endif
                enddo
            enddo
        enddo
        if (str % moltype .eq. PROT.or.str % moltype .eq. COMPLEX) then
            ...
            do i = 1, str % nmol
                ...
                if (oxtInd .gt. 0) then
                    str % bonds(findAtomInRes(str, str % mols(i) % fin, 'C'), findAtomInRes(str, str % mols(i) % fin, 'OXT')) = COVB
                    str % bonds(findAtomInRes(str, str % mols(i) % fin, 'O'), findAtomInRes(str, str % mols(i) % fin, 'OXT')) = COVB
                endif
            enddo
        endif
    ...
    subroutine setFrozenBonds(str, recNatoms, cutoff)
        ...
        do i = 1, recNatoms - 1
            do j = i + 1, recNatoms
                if (str % ats(i) % frozen.and.str % ats(j) % frozen.and.str % distat2(i, j) .le. c2) str % bonds(i, j) = COVF
            enddo
        enddo
    ...
    subroutine setPeptideBonds(str, crossb)
        ...
        do nr = 1, str % nres - 1
            if (crossb.or.str % res(nr) % molres .eq. str % res(nr + 1) % molres) then
                str % bonds(str % res(nr) % ico, str % res(nr + 1) % in)  = COVB ! C-N
                str % bonds(str % res(nr) % ica, str % res(nr + 1) % ica) = COVF ! CA-(C)-(N)-CA
                str % bonds(str % res(nr) % ica, str % res(nr + 1) % in)  = COVF ! CA-(C)-N
                str % bonds(str % res(nr) % ico, str % res(nr + 1) % ica) = COVF ! C-(N)-CA
                str % bonds(findAtominRes(str, nr, 'O'), str % res(nr + 1) % in) = COVF ! O-(C)-N
            endif
        enddo
                ...
                do nr2 = nr1 + 1, str % nres
                    if (eqId(str % res(nr2) % resId, 'CY_')) then
                        ...
                        if (sg1 .ne. 0.and.sg2 .ne. 0) then
                            if (str % distat2(sg1, sg2) .le. rssmax2) then
                                str % bonds(sg1, sg2) = COVB ! SG-SG
                                str % bonds(cb1, sg2) = COVF ! CB-(SG)-SG
                                str % bonds(sg1, cb2) = COVF ! SG-(SG)-CB
                            endif
                        endif
                    endif
                enddo
    ...
    subroutine setHBonds(str)
        ...
        integer i
        integer lloc(1)
        ! hbonds main chain, el m�s proper possible es i, i+3 en hairpins posem i i+4 per compatibilitat
    ...
    subroutine prepBondList(str)
        type(struc), intent(INOUT) :: str
        integer i, j
        ! anulem enlla�os amb membres dummy
        do i = 1, str % natoms
            if (str % ats(i) % dummy) then
                str % bonds(i, 1:str % natoms) = NULL
                str % bonds(1:str % natoms, i) = NULL
            endif
        enddo
        !
        str % cov = allocatePairList(count(str % bonds .eq. COVB) + count(str % bonds .eq. COVF))
        ...
        do i = 1, str % natoms - 1
            do j = i + 1, str % natoms
                if (str % bonds(i, j) .eq. COVB.or.str % bonds(i, j) .eq. COVF) then
    ...
    function checkHBNO(str, resN, atN, resO, atO) result(hb)
        ...
        if (resN .gt. 1.and.resO .ne. 0.and.atN .ne. 0.and.atO .ne. 0) then
            ...
            str % distat2(str % res(resN - 1) % ico, atO) .gt. rcomin2.and. &
            str % distat2(str % res(resN - 1) % ico, atO) .lt. rcomax2.and. &
            str % distat2(str % res(resN) % ica, atO) .gt. rcomin2.and. &
            str % distat2(str % res(resN) % ica, atO) .lt. rcomax2.and. &
            str % distat2(atN, str % res(resO) % ico) .gt. rncmin2.and. &
            str % distat2(atN, str % res(resO) % ico) .lt. rncmax2)
        endif
    ...
    subroutine calcSecStr(str) ! calculem SS amb el patro de ponts d'hidrogen
        ...
        do while (i .le. str % hbs % npairs)
            if ((str % ats(str % hbs % list(i, 2)) % resnum - str % ats(str % hbs % list(i, 1)) % resnum) .eq. 4) then
                j = i
                do while (j .le. str % hbs % npairs.and. &
                    (str % ats(str % hbs % list(j, 2)) % resnum - str % ats(str % hbs % list(j, 1)) % resnum) .eq. 4 .and. &
    ...
    !===============================================================================
    subroutine setStepPotentials(str, stL, ff, fvdw, fsolv)
        use potentials
        type (struc), intent(INOUT) :: str
        type (ffPrm), intent(IN) :: ff
        type (stepPotIntDefList), intent(IN) :: stL
        real, intent(IN) :: fsolv, fvdw
        call setStepPotCOV(str,StL)
        call setStepPotSS(str,stL)
        call setStepPotNB(str, stL, ff, fvdw, fsolv)
    end subroutine setStepPotentials
    !===============================================================================
    subroutine setStepPotCOV(str, stL)
        type (struc), intent(INOUT) :: str
        type (stepPotIntDefList), intent(IN) :: stL
        integer i, j, k
        !
        do i = 1, str % cov % npairs
            j = str % cov % list(i, 1)
            k = str % cov % list(i, 2)
            str%iData(j,k) = intDataTop(0,COVB,sqrt(str % distat2(j, k)),0.)
            if (str % bonds(i, j) .eq. COVB) then
                str % iData(j,k)%id = getstepPotDefIndex (stL, 'COVB')
            else
                str % iData(j,k)%id = getstepPotDefIndex (stL, 'COVF')
            endif
        enddo
        do i = 1, str % hbs % npairs
            j = str % hbs % list(i, 1)
            k = str % hbs % list(i, 2)
            str%iData(j,k) = intDataTop(getStepPotDefIndex(stL,'HB'),HB,0.,0.)
            if (str % ats(j) % atomId .eq. 'N'.or.str % ats(k) % atomId .eq. 'N') then
                str % iData(j,k)%rref = 0.5 * (rnomax + rnomin)
            else
                str % iData(j,k)%rref = 0.5 * (rcomax + rcomin)
            endif
        enddo
    end subroutine setStepPotCOV
    !===============================================================================
    subroutine setStepPotSS(str, stL)
        type (struc), intent(INOUT) :: str
        type (stepPotIntDefList), intent(IN) :: stL
        integer i, j, k

        do i = 1, str % nhelix
            do j = str % helix(i) % ini + 1, str % helix(i) % fin
                call setbondType(str,str % res(j - 1) % ico, str % res(j) % ico, SSEC)
            enddo
            do j = str % helix(i) % ini, str % helix(i) % fin - 1
                call setBondType(str, str % res(j) % in, str % res(j + 1) % in, SSEC)
                do k = j + 1, str % helix(i) % fin
                    call setBondType(str, str % res(j) % ica, str % res(k) % ica, SSEC)
                enddo
            enddo
        enddo
        do i = 1, str % nbeta
            do j = str % beta(i) % ini + 1, str % beta(i) % fin
                call setBondType(str, str % res(j - 1) % ico, str % res(j) % ico, SSEC)
            enddo
            do j = str % beta(i) % ini, str % beta(i) % fin - 1
                call setBondType(str, str % res(j) % in, str % res(j + 1) % in, SSEC)
                ! RCUTGO fixat a 20A modificable a run-time
                do k = j + 1, str % beta(i) % fin
                    if (str % distat2(str % res(j) % ica, str % res(k) % ica) .lt. RCUTGO2) &
                    call setBondType(str,str % res(j) % ica, str % res(k) % ica, SSEC)
                enddo
            enddo
        enddo
        do j = 2, str % natoms
            do i = 1, j - 1
                if (str % bonds(i, j) .eq. SSEC) &
                str % iData(i, j) = intDataTop(getStepPotDefIndex(stL,'SS'),SSEC,sqrt(str % distat2(i, j)), 0.)
            enddo
        enddo
    end subroutine setStepPotSS
    !===============================================================================
    subroutine setBondType(str, a1i, a2i, btype)
        type(struc), intent(INOUT) :: str
        integer, intent(IN) :: a1i, a2i
        integer, intent(IN) :: btype
        if (a1i .gt. 0.and.a2i .gt. 0) then
            if (str % bonds(a1i, a2i) .eq. NULL) str % bonds(a1i, a2i) = btype
        endif
    end subroutine setBondType        
    !===============================================================================
    subroutine setStepPotNB(str, stL, ff, FSOLV, FVDW)
        use potentials
        type (struc), intent(INOUT) :: str
       type (stepPotIntDefList), intent(IN) :: stL
        type (ffPrm), intent(IN) :: ff
        ...
        integer i, j
        type(attype) ati, atj
        real rvdwij, ecoul, esolv, potlk
        do j = 2, str % natoms
            do i = 1, j - 1
                if (str % distat2(i, j) .lt. RCUTNB2) then
                    ati = getPotential(ff, str % ats(i) % atType)
                    atj = getPotential(ff, str % ats(j) % atType)
                    if (str % bonds(i, j) .eq. NULL)then
                        esolv = -sqrt(ati % evdw * atj % evdw) * FVDW
                        rvdwij = ati % rvdw + atj % rvdw
                        potlk = ati % gfree * atj % vol / ati % xlamb + atj % gfree * ati % vol / atj % xlamb
                        esolv = esolv - potlk / rvdwij**2 * 0.09 * FSOLV
                        ecoul = ati % qq * atj % qq
                        if (abs(ecoul) .gt. MINREAL) then
                            str % iData(i, j) = intDataTop(0, COUL, rvdwij, ecoul)
                            if (ecoul .lt. 0.) then
                                 str % iData(i, j)%id = getStepPotDefIndex(stL,'COUN')
                            else
                                str % iData(i, j)%id = getStepPotDefIndex(stL,'COUP')
                            endif
                        else
                            if (esolv .lt. 0.) then
                            str % iData(i, j) = intDataTop(getStepPotDefIndex(stL,'HFOB'), HFOB, rvdwij, esolv)
                            else
                            str % iData(i, j) = intDataTop(getStepPotDefIndex(stL,'HFIL'), HFIL, rvdwij, esolv)
                            endif
                        endif
                    endif
                endif
            enddo
        enddo
    end subroutine setStepPotNB
    !===============================================================================
    subroutine saveTopology(unt, str, stL, ff)
        use potentials
        integer, intent(IN) :: unt
        type(struc), intent(IN) :: str
        type(ffprm), intent(IN) :: ff
        ...
        type(atType) at
        integer i, j
        write (unt) itopVersion
        write (unt) str % moltype
        write (unt) str % nmol, str % nres, str % natoms
        write (unt) (str % ats(i) % atomId, str % ats(i) % resnum, str % ats(i) % resIdByAtom, str % ats(i) % chainId, &
        str % ats(i) % atType, str % ats(i) % frozen, str % ats(i) % dummy, i = 1, str % natoms)
        do i = 1, str % natoms
            at = getPotential(ff, str % ats(i) % atType)
            write (unt) at%qq, at % evdw, at % rvdw, at % rhc, at % mas
        enddo
        write (unt) stL%npots
        do i=1,stL%npots
        write (unt) stL%list(i)
        enddo
        do i = 1, str % natoms - 1
            if (.not.str % ats(i) % dummy) then
                do j = i + 1, str % natoms
                    if (.not.str % ats(j) % dummy.and.str % iData(i, j) % tipInt .ne. NULL) &
                    write (unt) i, j, str % iData(i, j)
                enddo
            endif
        enddo
